...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    document.getElementById("wave-info").textContent = wvht
      ? `${wvht} m${dpd ? ` @ ${dpd}s` : ""}`
      : "No wave data";
    document.getElementById("wind-info").textContent = wspd
      ? `${wspd} m/s${wdir ? ` dir ${wdir}°` : ""}`
      : "No wind data";
    document.getElementById("air-temp").textContent = atmp
      ? `${atmp} °C`
      : "N/A";
    document.getElementById("water-temp").textContent = wtmp
      ? `${wtmp} °C`
      : "N/A";
    ...
    if (colIndex["WVHT"] !== undefined) {
      ...
      const waveHeights = rows.map((r) => {
        ...
        return val ? parseFloat(val) : null; // null instead of 0
      });
          ...
          datasets: [
            {
              ...
              backgroundColor: waveHeights
                .map((h) => {
                  if (h === null) return "rgba(0,0,0,0)"; // invisible
    ...
    document.getElementById("buoy-data").innerHTML = `<p style="color:red;">Error loading buoy data: ${err.message}</p>`;
